...
import { exportModelToPNML } from "./util/PNMLGenerator";
...
export default class BaseViewer extends Diagram {
  ...
  savePNML(options) {
    ...
    return new Promise(function (resolve, reject) {
      let err, pnml;
      if (!definitions) {
        err = new Error("no xml loaded");
        return reject(err);
      }
      
      // allow to fiddle around with definitions
      definitions =
      self._emit("savePNML.start", {
        definitions: definitions,
      }) || definitions;
      ...
      const myModel = {
        ...
        version: "1.0.4",
        definitions: [
          { id: "def_1", type: "User", active: true },
          { id: "def_2", type: "Admin", active: false }
        ]
      };

      try {
        pnml = exportModelToPNML(definitions);
      } catch (e) {
        err = e;
      }

      self._emit("savePNML.done", {
        error: err,
        pnml: pnml,
      });

      if (!err) {
        return resolve({ pnml: pnml });
      }
