...
ToggleSimulationMode.prototype._init = function () {
  this._container = domify(`
    <div class="tpn-toggle-mode">
      Simulation Mode
      <span class="tpn-toggle">
        <img src="${generatedIcons["toggle-off"]}" alt="Toggle Simulation Mode" width="24" height="24"/>
      </span>
    </div>
  `);
...
ToggleSimulationMode.prototype.toggleMode = function (active = !this._active) {
  ...
  if (active) {
    this._container.innerHTML = `Simulation Mode <span class="tpn-toggle"><img src="${generatedIcons["toggle-on"]}" alt="Toggle Simulation Mode" width="24" height="24"/></span></span>`;

    domClasses(this._palette).add("hidden");
    domClasses(this._canvasParent).add("simulation");
  } else {
    this._container.innerHTML = `Simulation Mode <span class="tpn-toggle"><img src="${generatedIcons["toggle-off"]}" alt="Toggle Simulation Mode" width="24" height="24"/></span></span>`;
