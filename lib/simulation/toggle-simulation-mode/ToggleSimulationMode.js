...
ToggleSimulationMode.prototype.toggleMode = function (active = !this._active) {
  ...
  if (active) {
    this._container.innerHTML = `Simulation Mode <span class="tpn-toggle"><span class="pn-icon-toggle-on"></span></span>`;

    domClasses(this._palette).add("hidden");
    domClasses(this._canvasParent).add("simulation");
  } else {
    this._container.innerHTML = `Simulation Mode <span class="tpn-toggle"><span class="pn-icon-toggle-off"></span></span>`;
