    ...
    if (randomRequest) {
      ...
      validBindings.forEach((b) => {
        vars.forEach((v) => {
          ...
          if (values !== undefined) {
            // For variable arcs with multiple values, add all items individually
            if (values.length > 1) {
      ...
      const fields = Array.from(
        fieldsContainer.querySelectorAll(
          "select, .bts-fire-checkbox-list",
        ),
      );

      fields.forEach((s) => {
        // Skip multi-select
        if (s.classList.contains("bts-fire-checkbox-list")) return;
      ...
      const updateFilters = (changedSelect) => {
        ...
        fields.forEach((s) => {
          const name = s.getAttribute("data-var");
          if (!currentChoices[name]) currentChoices[name] = [];

          if (s.classList.contains("bts-fire-checkbox-list")) {
            const selected = Array.from(s.querySelectorAll("input[type='checkbox']:checked")).map((o) => o.value);
        ...
        // for each field, compute allowed values based on current choices excluding itself
        fields.forEach((s) => {
          const name = s.getAttribute("data-var");
          const allowed = new Set();

          validBindings.forEach((b) => {
            // check compatibility with all other fields (excluding this field)
            ...
            if (isCompatible(otherChoices, b)) {
              if (b[varsToDataClassKey[name]] !== undefined) {
                const values = Array.isArray(b[varsToDataClassKey[name]])
                  ? b[varsToDataClassKey[name]]
                  : [b[varsToDataClassKey[name]]];
                values.forEach((val) => allowed.add(String(val)));
              }
            }
          });

          if (s.classList.contains("bts-fire-checkbox-list")) {
            // Handle Checkbox list dynamic rebuilding
            const curChecked = Array.from(s.querySelectorAll("input[type='checkbox']:checked")).map((o) => o.value);
            const sortedAllowed = Array.from(allowed).filter((val) => val !== "").sort();
            s.innerHTML = "";

            if (sortedAllowed.length === 0) {
              const noOptionNotice = document.createElement("div");
              noOptionNotice.textContent = "no viable option";
              noOptionNotice.style.color = "#ff4d4f"; 
              noOptionNotice.style.fontStyle = "italic";
              noOptionNotice.style.fontSize = "12px";
              noOptionNotice.style.padding = "4px 2px";
              noOptionNotice.style.width = "100%";
              noOptionNotice.style.boxSizing = "border-box";
              s.appendChild(noOptionNotice);
            } else {
              sortedAllowed.forEach((val) => {
                ...
                s.appendChild(itemWrapper);
              });
            }
          } else {
            // Original select dropdown rebuilding logic
            const cur = [s.value];
            ...
            if (cur[0] && Array.from(s.options).some((o) => o.value === cur[0])) {
              s.value = cur[0];
            } else {
              s.value = "";
            }
      ...
      // Update filters on each field change event (bubbles natively for checkboxes)
      fields.forEach((s) => {
      ...
      // Enable confirm button when all interactive elements have a valid bound value selection
      updateConfirmEnabled = () => {
        if (!confirmBtn) return;
        const allBound = fields.every((s) => {
          const name = s.getAttribute("data-var");
          if (s.classList.contains("bts-fire-checkbox-list")) {
            const checkboxes = Array.from(s.querySelectorAll("input[type='checkbox']"));
            if (varsToDataClassKey[name].endsWith(":exact")) {
              return checkboxes.length > 0 && checkboxes.every((o) => o.checked);
            }
            return checkboxes.some((o) => o.checked);
          }
