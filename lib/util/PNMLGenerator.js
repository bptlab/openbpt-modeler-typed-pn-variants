...
function unclutter(modelElements) {
  ...
  const transitions = modelElements.filter(
    (el) => el.$type === "tpn:Transition",
  );
...
export function exportModelToPNML(definitions) {
  const { places, transitions, arcs } = unclutter(
    definitions.model.modelElements,
  );
  const modelName =
    definitions.model.name ||
    "This is a PNML representation of the (D)OPID model.";
  ...
  for (const place of places) {
    ...
    if (place.marking) {
      ...
      markingText.appendChild(
        doc.createTextNode(place.marking.length.toString()),
      );
  ...
  for (const arc of arcs) {
    if (arc.isInhibitorArc) continue; // Skip inhibitor arcs for now, as PNML does not have a standard way to represent them
    ...
    const inscription = (arc.inscription.inscriptionElements || [])
      .map((i) => {
        const variableName = i.isGenerated ? "nu" : i.variableName;
        const typeName =
          i.dataClass.name === variableType
            ? i.dataClass.name.toUpperCase() + " LIST"
            : i.dataClass.name.toUpperCase();
