...
import { isStructurallyIncorrect } from "../simulation/binding-utils/earlyReturnLogic";
...
export default class CustomVerification extends CommandInterceptor {
  constructor(eventBus, overlays, elementRegistry) {
    ...
    function verifyAfterElementUpdate(e) {
      ...
      if (!contextArc?.type || contextArc.type !== `${MODELER_PREFIX}:Arc`)
        return;
  ...
  verifyStructuralCorrectness = function (self, transition) {
    const [isIncorrect, message] = isStructurallyIncorrect(
      transition.incoming,
      transition.outgoing,
    );
      ...
      const overlayId = self._overlays.add(transition, {
        ...
        html: `
					<div class="structurally-incorrect-overlay" style="cursor: pointer; overflow: visible; font-size: 25;">
						<div class="structurally-incorrect-tooltip">${escapedMessage}</div>
					<svg width="24px" height="24px" viewBox="0 0 20 20" style="overflow: visible;">
						<circle cx="10" cy="10" r="10" fill="#fff3cd" stroke="#856404" stroke-width="0.15em"/>
						<text x="10" y="14" text-anchor="middle" font-size="16" fill="#856404" font-family="Arial">⚠</text>
					</svg>
					</div>
				`,
      });
...
CustomVerification.$inject = ["eventBus", "overlays", "elementRegistry"];
